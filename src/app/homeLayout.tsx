...
const HomeLayout = ({ handlebgloadingFunc }: IProps) => {
  ...
  const subTotal = carts?.data
    ?.map((c: any) => parseFloat(c?.menu?.price) || 0)
  ...
  return (
    <>
        ...
        <div className="flex justify-between items-center w-[90%] mx-auto">
          <div className="flex gap-4 items-center ">
            <div className="flex-content">
              <div
                className="flex sm:flex md:hidden lg:hidden xl:hidden"
                onClick={() => handleMenu()}
              >
                <MenuIcon color="green" stroke="green" />
              </div>
              <Link href="/" className="font-bold text-[1.3rem]">
                Foodie
              </Link>
            </div>
            <nav className="hidden sm:hidden md:flex md:gap-4 lg:flex lg:gap-4">
              ...
              <Link href="/menu">
                {" "}
                <h3 className="link-style">Recommendation</h3>
              </Link>
            </nav>
          </div>
      ...
      <ModalComponent1
        title={logStyle.show ? "SignIn" : "SignUp"}
        open={modal?.show}
        width={400}
        center={false}
      ...
      >
        <div className="text-center" onClick={() => setMenu({ show: false })}>
          <Link href="/">
            <h3 className="text-center py-1 font-bold">Home</h3>
          </Link>

          <Link href="/menu">
            {" "}
            <h3 className="text-center  py-1 font-bold">Recommendation</h3>
          </Link>
        </div>
      </ModalComponent2>
    </>
